...
def addToList(self):
    ...
    var_fname = var_fname.strip() # This will remove any blank spaces before an after the user's entry
    var_lname = var_lname.strip()
    var_fname = var_fname.title() # This wilil ensure that the first character in each word is capitalized
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # force the user to provide all info
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel('Update Request','The following changes ({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?'.format(var_phone,var_email,var_value))
                print(response)
                if response:
                    # conn already defined
                    with conn:
                        cursor = conn.cursor()
